...
function gateKeeper(req, res, next) {
  const params = queryString.parse(req.headers['x-username-and-password']);
  
  if(params.user && params.pass) { //we got the header with a user and pass
    const user = USERS.find(element => element.userName === params.user);
    if(user && user.password === params.pass) {  //a matching user is found and the password matches
      req.user = user;
    } else {
      req.user; //add the element but do not assign a value
    }
  }
